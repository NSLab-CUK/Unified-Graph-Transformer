...
def addPE(og_data, PE):
	...
	#A = dglsp.spmatrix(edge_index, shape=(131, 131))
...
def getI(nx_g, k_hop):
	...
	#if target_node ==0 or target_node ==1 :
...
def checkingVirtualEdges(G_og, G_st, G_all, data):
	...
	# freq2
	# with open(r'degreeR.txt', 'w') as fp:
	# 	for item in freq_1:
	# 		fp.write("%s\n" % item)

	# with open(r'degreeV.txt', 'w') as fp:
	# 	for item in freq2:
	# 		fp.write("%s\n" % item)
...
def normalizeTensor(x):
    ...
    return x_normed
def g_dgl(data, PE,  Kindices, D,  D_dim, M, I, pos_enc_size):
	...
	PE = dgl.lap_pe(g1, pos_enc_size, padding=True)
	...
	for i in range(len(edge_idx1)):
		...
		d_vals.append(d)
		#
		# k_edge_idx1.append(n2)
		# k_edge_idx2.append(n1)
		#
		# d_vals.append(d)
		# m_vals.append(m)

	# print(f'shape distance matrix list: {np.shape(d_vals)}')
	# print(f'shape M matrix list: {np.shape(m_vals)}')

	# d_vals[np.isnan(d_vals)] = 0
	# m_vals[np.isnan(m_vals)] = 0
	...
	#ew = torch.tensor(vals)
	g = dgl.graph((k_edge_idx1, k_edge_idx2))
	#print(f'number K edges index: {count}')
	g = dgl.to_bidirected(g)

	if torch.any(torch.isnan(I)):
		print(f'I1: {I}, size: {I.size()}')
	...
	# dgl.lap_pe(g, k=2, padding=True)
	# print(f'shape Kin: {np.shape(Kindices)}')
	# g.ndata["KI"] =  torch.LongTensor(Kindices)
	return g
...
def getM_logM(nx_g, num_nodes, k_transition):
	...
	# save adj matrix to modularity modulo:
	#print(f'getM_logM: # Zero {zerro_count}')
	...
	for i in range(8):
		...
		#print(f'transition matrix: {probTranMat}')
		# with open(r'transition_matrix.txt', 'w') as fp:
		# 	for item in probTranMat:
		# 		fp.write("%s\n" % item)
		# raise SystemExit(0)
		#print(probTranMat)
	#print(f'tran_logM: {np.shape(tran_logM)}')
	...
	for src in nx_g.nodes():
		for dst in nx_g.nodes():
			...
			# print(f'trans: {trans}')
			# print(f'len trans: {np.shape(trans)}')
			M[src][dst] = trans
			#raise SystemExit(0)
...
#construct laplacian eigenvector
def buildPE_Kindices(data1,dim,trans_M,k_hop):
	#print('building PE k indicaces ')
	#dim = 3  # k_eigenvector
	...
	#Eig = np.zeros((num_nodes, num_nodes, dim + 2))
	Eig = dgl.lap_pe(g, dim,padding=True)

	#Eig_cp = np.zeros((num_nodes, num_nodes, dim))
	#EigVecs = dgl.laplacian_pe(g,dim)
	#EigVecs = dgl.lap_pe(g, dim,padding=True)

	Eig += torch.full_like(Eig, 1e-5); Eig = normalizeTensor(Eig)

	#Eig = np.abs(normalize(Eig, axis=1, norm='l1'))

	


	#print(f'dgl eigVecs shape: {np.shape(Eig)}')
	...
	#print('checking Eigenvectors Laplacian . just remove comment and return ...')
...
# def get_edge_index(nodes_ids, all_path):
# 	src = []
# 	dst = []
# 	for s_idx, s_node in enumerate(nodes_ids):
# 		spd_from_idx = all_path[s_idx]
...
# 				continue
# 			else:
# 				if s_node in dst and target_node in src:
# 					continue
# 				src.append(s_node)
# 				dst.append(target_node)
# 				#print(f's,d {s_node, target_node}')
