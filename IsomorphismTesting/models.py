...
class Transformer(nn.Module):
	def __init__(self, in_dim, out_dim, pos_enc_size,  hidden_dim, num_layers, num_heads, D_dim, k_transition, num_classes):
		...
		self.fc1 = torch.nn.Linear(out_dim, 10)
	def extract_features(self, g, batch_x, batch_PE, batch_de, batch_m, batch_I ):
		...
		dis_E = batch_de
		I = batch_I
		lap = self.lap_pos_enc(lap.float())
		h = self.embedding_h(X)	  + lap
	...
	def forward(self, batch_g, batch_x, batch_PE, batch_de, batch_m, batch_I ):
		...
		self.h = h
		hg = dgl.mean_nodes(batch_g, 'h') 
		hg = torch.tanh(self.fc1(hg))
		return hg
...
class GraphTransformerLayer(nn.Module):
	...
	def forward(self, h, g, de, m, I ):
		...
		h = h + self.proj_i(I.float()) # + torch.full_like(I, 1e-5) # adding eps to all values here
		h = self.O(h)
...
class MultiHeadAttentionLayer(nn.Module):
	...
	def forward(self, h, g, de, m):
		...
		h_out = g.ndata['wV'] / (g.ndata['z'] + torch.full_like(g.ndata['z'], 1e-10)) # adding eps to all values here
