...
def build_multigraph_from_layers(networkx_graph, y, x=None, edge_index_og= None):
	...
	#1. Remove same edge: structural - original edges
	ind = []
	count = 0
	...
	num_of_edges_virtual = int( 1  * num_of_edges / 10)
	ind = []
	vals = []
	...
	count = 0
	l_label = []
	total = len(edge_weight)
	for i in range(total):
		node_1 = edge_index[0,i]
		node_2 = edge_index[1,i]
		if y[node_1] == y[node_2]:
			count+= 1
			l = int(y[node_1])
			l_label.append(l)
